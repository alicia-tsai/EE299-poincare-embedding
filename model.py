...
class EuclideanDistance(nn.Module):
    ...
    def forward(self, u, v):
        return th.sqrt(th.sum(th.pow(u - v, 2), dim=-1))
...
class SNEmbedding(Embedding):
    ...
    def _forward(self, e):
        o = e.narrow(1, 1, e.size(1) - 1)   # remove frist column
...
class GraphDataset(Dataset):
    ...
    def __init__(self, idx, objects, nnegs, unigram_size=1e8):
        ...
        for i in range(idx.size(0)):   # row
            t, h, w = self.idx[i]
            t, h, w = int(t), int(h), int(w)  # convert to integer
            self._counts[h] += w
            self._weights[t][h] += w
...
class SNGraphDataset(GraphDataset):
    ...
    def __getitem__(self, i):
        t, h, _ = self.idx[i]
        t, h = int(t), int(h)  # convert to integer
        negs = set()
        ntries = 0
        nnegs = self.nnegs
        if self.burnin:
